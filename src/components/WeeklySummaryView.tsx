...
import { calculateWeeklySummary } from '../services/dataService';
    ...
    return (
      <div className="p-8 rounded-3xl bg-[#101010] border border-[#222222] text-center space-y-4 max-w-xl mx-auto my-12 shadow-2xl animate-fadeIn">
        ...
        <p className="text-xs text-slate-400 leading-relaxed font-medium">
          Cadastre seus escritórios no menu <b>Escritórios & Metas</b> para acompanhar o resumo semanal comparativo.
        </p>
      </div>
    );
  }

  const weeklyData = calculateWeeklySummary(baseData, offices);
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-dark-800 border border-slate-800 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-white tracking-tight">Resumo Semanal — Comparativo de Desempenho</h2>
            <p className="text-xs text-slate-400">Comparação da semana atual (segunda a domingo) com a semana anterior.</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400 uppercase text-[10px] tracking-wider">
                ...
                <th className="py-3.5 px-4 font-bold">BOLETOS SEMANA ATUAL</th>
                <th className="py-3.5 px-4 font-bold">BOLETOS SEMANA ANTERIOR</th>
                <th className="py-3.5 px-4 font-bold">VARIAÇÃO %</th>
                <th className="py-3.5 px-4 font-bold">CONTAS SEMANA ATUAL</th>
                <th className="py-3.5 px-4 font-bold">CONTAS SEMANA ANTERIOR</th>
                <th className="py-3.5 px-4 font-bold">VARIAÇÃO %</th>
                <th className="py-3.5 px-4 font-bold">CONVERSÃO ATUAL</th>
                <th className="py-3.5 px-4 font-bold">CONVERSÃO ANTERIOR</th>
                <th className="py-3.5 px-4 font-bold">VARIAÇÃO (P.P.)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60 font-medium">
              {weeklyData.map((row, idx) => {
                const isTotal = row.escritorio === 'TOTAL GERAL';
                ...
                return (
                  <tr key={idx} className={`hover:bg-[#161616] transition-colors ${isTotal ? 'bg-[#141414] font-black text-sm border-t-2 border-[#222222] text-white' : 'text-slate-200'}`}>
                    <td className="py-4 px-4 font-bold text-white flex items-center gap-2">
                      {!isTotal && <span className="w-2.5 h-2.5 rounded-full bg-brand-yellow" />}
                      <span>{row.escritorio}</span>
                    </td>
                    
                    <td className="py-4 px-4 text-purple-300 font-extrabold">{row.boletosAtual.toLocaleString('pt-BR')}</td>
                    <td className="py-4 px-4 text-slate-400">{row.boletosAnterior.toLocaleString('pt-BR')}</td>
                    ...
                    <td className="py-4 px-4 text-cyan-300 font-extrabold">{row.contasAtual.toLocaleString('pt-BR')}</td>
                    <td className="py-4 px-4 text-slate-400">{row.contasAnterior.toLocaleString('pt-BR')}</td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4 text-emerald-400 font-extrabold">{(row.conversaoAtual * 100).toFixed(2)}%</td>
                    <td className="py-4 px-4 text-slate-400">{(row.conversaoAnterior * 100).toFixed(2)}%</td>
                    <td className="py-4 px-4">
